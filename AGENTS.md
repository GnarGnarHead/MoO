...
- `python3 constructionist_math.py --stats ...` — prints `Graph.stats()` and `Graph.snap_events()` before JSON/DOT.
- `python3 constructionist_math.py --maps ...` — convenience flag for `--stats --snap-dot --field --field-ascii`.
- `python3 constructionist_math.py --write-maps out/demo --limit 6` — writes `out/demo.*` map files (DOT + snap-DOT + field CSV/JSON/ASCII + stats).
- If Graphviz is available, render maps with: `dot -Tsvg out/demo.snap.dot -o out/demo.snap.svg`.
...
- Avoid silent mutation of shared structures; prefer helper methods (e.g., `_record_edge`, `_snap_speculative_to_ref`, `_maybe_snap_new_spec_to_existing_ref`) to keep graph integrity rules centralized.

## Testing Guidelines
- There is no formal test suite yet; rely on the demo run for quick validation.
- When adding features, create lightweight checks that exercise grounded vs speculative paths (e.g., division by zero, zero-annihilation multiplication, speculative promotion to grounded, and S→G snapping telemetry).
